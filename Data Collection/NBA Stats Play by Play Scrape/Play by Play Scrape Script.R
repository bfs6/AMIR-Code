...
####Get URLs for 1998 to 2018####
pbp_urls = {}
for(year in c(1998:2018)){
  game_id_vec <- c(1:1230)
  year_pbp_list <- list()
  for(x in game_id_vec){
    pbp_base_url <- "http://stats.nba.com/stats/playbyplayv2/?GameID="
    game_id <- paste0("002", substr(year, 3, 4), sprintf("%05d", x))
    print(game_id)
    game_list <- list()
    pbp_url <- paste0(pbp_base_url, game_id, "&StartPeriod=", 1, "&EndPeriod=", 8)
    pbp_urls = rbind(pbp_urls,pbp_url)
  }
}
