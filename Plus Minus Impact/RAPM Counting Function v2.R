...
rapm_counting_function <- function(year){
  ...
  for(game_id in seq_along(game_list)){
    ...
    ##Fix Score Margin To Be Numeric
    game$SCOREMARGIN_numeric <- rep(NA, nrow(game))
    game$SCOREMARGIN_numeric[1] = 0
    for(score in 2:nrow(game)){
      if(is.na(game$SCOREMARGIN[score]) == T){
        game$SCOREMARGIN_numeric[score] = game$SCOREMARGIN_numeric[score - 1]
      }else if (game$SCOREMARGIN[score] == "TIE"){
        game$SCOREMARGIN_numeric[score] = 0
      }else{
        game$SCOREMARGIN_numeric[score] = as.numeric(game$SCOREMARGIN[score])
      }
    }
    
    ##Garbage Time Definitions
    awayplayersStart <- sort(unique(unlist(game[1, paste0("Away_Player_", c(1:5))])))
    homeplayersStart <- sort(unique(unlist(game[1, paste0("Home_Player_", c(1:5))])))
    game$PCTIMESTRING_round <- as.numeric(sapply(1:nrow(game), function(z) strsplit(game$PCTIMESTRING, ":")[[z]][1]))
    game$garbage_time <- rep(0, nrow(game))
    game$starterCount <- rep(NA, nrow(game))
    for(garbage in seq_along(game$garbage_time)) {
      awayplayers <- sort(unique(unlist(game[garbage, paste0("Away_Player_", c(1:5))])))
      homeplayers <- sort(unique(unlist(game[garbage, paste0("Home_Player_", c(1:5))])))
      bothHome <- homeplayersStart %in% homeplayers
      bothAway <- awayplayersStart %in% awayplayers
      starterCount <- length(which(bothHome == T)) + length(which(bothAway == T))
      game$starterCount[garbage] = starterCount
      if(game$PERIOD[garbage] == 4 & starterCount <= 2) {
        if(abs(game$SCOREMARGIN_numeric[garbage]) >= 25 & game$PCTIMESTRING_round[garbage] %in% c(9:12)){
          game$garbage_time[garbage] <- 1
        }else if(abs(game$SCOREMARGIN_numeric[garbage]) >= 20 & game$PCTIMESTRING_round[garbage] %in% c(6:8)) {
          game$garbage_time[garbage] <- 1
        }else if(abs(game$SCOREMARGIN_numeric[garbage]) >= 10 & game$PCTIMESTRING_round[garbage] %in% c(0:5)) {
          game$garbage_time[garbage] <- 1
        }
      }
    }
    for(garbage in 1:(nrow(game) - 1)){
      if(game$PERIOD[garbage] == 4){
        if(game$garbage_time[garbage] == 0){
          game$garbage_time[1:(garbage-1)] = rep(0, garbage-1)
        }
      }
    }
    
    ##Create Shots and Get Rid of Garbage Time
    shots <- game[sort(c(which(game$EVENTMSGTYPE %in% c(1:3, 5)), which(game$EVENTMSGTYPE == 4 & game$def_rebound == 1))),]
    shots <- shots[-which(shots$garbage_time == 1), ]
